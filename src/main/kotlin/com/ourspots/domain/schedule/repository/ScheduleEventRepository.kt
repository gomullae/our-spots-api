...
interface ScheduleEventRepository : JpaRepository<ScheduleEvent, Long> {
    ...
    @Query("SELECT * FROM schedule_events WHERE created_at >= :cutoff ORDER BY id", nativeQuery = true)
    fun findAllIncludingDeletedSince(cutoff: LocalDateTime): List<ScheduleEvent>
